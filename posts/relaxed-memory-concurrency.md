...
内存一致性模型是系统和程序员之间的规范，规定了在多线程共享内存程序中，内存访问应表现出怎样的行为。它由以下两部分组成。

### Memory Ordering

指内存访问操作是否可能呈现与程序顺序不一致的执行顺序。具体包括：

- **Load-Load Order**：两个读操作是否会乱序
- **Load-Store Order**：读操作与后面的写操作是否会乱序
- **Store-Load Order**：写操作与后面的读操作是否会乱序
- **Store-Store Order**：两个写操作是否会乱序
- **Dependent Loads Order**：当第二个读的地址取决于第一个读的结果时，是否会乱序

### Store Atomicity

指一个处理器的写操作何时对其他处理器可见，从弱到强分为 4 级：

> 第 1 级 · Load Other's Store Early && Non-Causality（最弱）

- 写操作可被自己及部分其他处理器先看到，**不支持因果传递性**——不同处理器看到同一写序列的顺序可能不同，且因果链可能断裂
- 典型场景：NUMA 或 SMT 系统，Cache Coherence 消息因物理距离不同而到达时间不同

> 第 2 级 · Load Other's Store Early && Causality

- 同第 1 级，允许部分处理器先看到写操作，但支持因果传递性：线程1 写 A（Release）→ 线程2 看到 A（Acquire）后写 B（Release），则 线程3 看到 B（Acquire）后一定能看到 A（因果传递依赖于 Release/Acquire 配对）
- 典型场景：搭配 Acquire/Release Barrier 的系统

> 第 3 级 · Load Own Store Early

- **只有写者自己**能通过 store buffer 先看到自己的写，其他处理器在刷出之前都看不到，在刷出那一刻同时看到；写序列以相同顺序被所有处理器观察到
- 典型场景：TSO（x86）

> 第 4 级 · Atomic Store（最强）
