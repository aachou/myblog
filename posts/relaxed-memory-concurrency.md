...
date = "2026-06-25"
tags = ["memory Model", "concurrency", "lock-free", “consistency”]
...
内存一致性模型是系统和程序员之间的约定，规定了在多处理器共享内存程序中，内存操作应表现出怎样的行为。

SC（Sequential Consistency）Model：任何一次执行等价于所有线程的操作按某个全局全序依次执行，且每个线程内的操作在该全序中保持程序顺序。

可以简单理解为，任何一次执行结果表现得像不同线程以交错的方式访问内存，线程内保持代码顺序，所有线程看到相同的交错顺序。这意味着系统不允许任何形式的指令重排序，且所有线程对所有内存地址的读写顺序达成全局一致。

在 SC 的基础上逐步放宽一致性要求会得到各种**宽松内存模型（Relaxed Memory Model）**，本文讨论的就是基于宽松内存模型的并发。
...
指令重排序指内存操作实际执行的顺序与程序代码不一致，导致反直觉的结果。编译器会直接重排指令；硬件则通过 store buffer、乱序执行等机制让其他线程观察到如同指令被重排了一样的效果。重排序可以发生在不存在数据依赖和控制依赖的内存操作之间。
